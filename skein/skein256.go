...
func (s *skein256) Write(in []byte) (int, error) {
	...
	length := len(in)
	if length > StateSize256 {
		nn := length - (length % StateSize256)
		if nn == length {
			nn -= StateSize256
		}
		s.hashMessage(in[:nn])
