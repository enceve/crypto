...
func Test(t *testing.T) {
	for i, v := range vectors {
		...
		if len(buf) != len(ciphertext) {
			t.Fatalf("TestVector %d Seal: length of buf and ciphertext does not match", i)
		}
		for j, v := range ciphertext {
			if v != buf[j] {
				t.Fatalf("TestVector %d Seal:\nFound   : %s\nExpected: %s", i, hex.EncodeToString(buf), hex.EncodeToString(ciphertext))
			}
		}
		...
		if len(buf) != len(msg) {
			t.Fatalf("TestVector %d Open: length of buf and msg does not match", i)
		}
		for j, v := range msg {
			if v != buf[j] {
				t.Fatalf("TestVector %d Open:\nFound   : %s\nExpected: %s", i, hex.EncodeToString(buf), hex.EncodeToString(msg))
			}
		}
	}
}
